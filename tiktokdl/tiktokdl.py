...
class TikTok:
    ...
    def get_meta(self, url):
        ...
        self.video = TikTokVideo(
            id=video_struct["id"],
            permalink=video_struct["playAddr"],
    ...
    def download(self, fp=None):
        ...
        video = requests.get(
            self.video.permalink, headers=headers, cookies={
                "tt_webid_v2": self.tt_webid_v2}
        )
        if not video.ok:
            raise Exception(
                f"Uh oh, we didn't get the data back from the CDN. {video}")

        if fp:
            if not hasattr(fp, "write"):
                raise Exception(
                    f"Variable `fp` did not have attribute \"write\"")
